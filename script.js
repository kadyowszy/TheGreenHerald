(function() {
    ...
    const config = {
        ...
        news: [
            { id: 1, headline: "University Announces New Scholarship Program", link: "#news-item-1" },
            { id: 2, headline: "Basketball Team Wins Regional Championship", link: "#news-item-2" }
        ],
        display: {
    ...
    function formatEventDateParts(dateString) {
        ...
        try {
            ...
            const optionsFull = { weekday: 'short', year: 'numeric', month: 'short', day: 'numeric', hour: dateString.includes('T') ? 'numeric' : undefined, minute: dateString.includes('T') ? '2-digit' : undefined };
            const optionsMonth = { month: 'short' };
            const optionsDay = { day: '2-digit' };
            return { full: date.toLocaleDateString(undefined, optionsFull), month: date.toLocaleDateString(undefined, optionsMonth).toUpperCase(), day: date.toLocaleDateString(undefined, optionsDay) };
        } catch (error) { console.error("Error formatting date:", dateString, error); return fallback; }
    }
    ...
    function getComparableDate(googleDateObj) {
        ...
        } catch (error) { console.error("Error creating comparable date:", googleDateObj, error); return null; }
    }
         ...
         for (let i = 0; i < config.display.maxEventsForHeroBlocks; i++) {
            ...
            if (monthEl && dayEl && titleEl) {
                ...
                if (event) {
                    ...
                    monthEl.textContent = dateParts.month; dayEl.textContent = dateParts.day; titleEl.textContent = eventTitle;
                } else {
                    monthEl.textContent = '---'; dayEl.textContent = '--'; titleEl.textContent = 'No Event';
                }
            } else { console.warn(`Hero block elements not found for index: ${blockIndex}`); }
        }
    }
    ...
    async function fetchCalendarEvents(calendarId) {
        ...
        try {
            ...
            if (!response.ok) { const errorText = await response.text(); console.error(`Error fetching calendar ${calendarId}: ${response.status} ${response.statusText}`, errorText); return []; }
            const data = await response.json(); const items = data.items || [];
            items.forEach(item => item.calendarSource = calendarId); return items;
        } catch (error) { console.error(`Network/Fetch error for calendar ${calendarId}:`, error); return []; }
    }
    ...
    async function fetchWeatherData() {
        ...
        try {
            const response = await fetch(apiUrl);
            if (!response.ok) {
                let errorDetails = ''; try { const errorData = await response.json(); errorDetails = errorData?.Message || response.statusText; } catch (e) { errorDetails = response.statusText; }
                console.error(`Error fetching AccuWeather: ${response.status} ${errorDetails}`); return null;
            }
            const data = await response.json();
            if (!data || !Array.isArray(data) || data.length === 0) { console.error('AccuWeather response format unexpected or empty:', data); return null; }
            const currentConditions = data[0];
            const tempMetric = currentConditions.Temperature?.Metric?.Value; const tempImperial = currentConditions.Temperature?.Imperial?.Value;
            const feelsLikeMetric = currentConditions.RealFeelTemperature?.Metric?.Value; const feelsLikeImperial = currentConditions.RealFeelTemperature?.Imperial?.Value;
            const weatherText = currentConditions.WeatherText; const weatherIcon = currentConditions.WeatherIcon;
            ...
            return { temp: displayTemp, unit: displayUnit, feelsLike: displayFeelsLike, description: weatherText, icon: weatherIcon, location: config.weather.locationName };
        } catch (error) { console.error('Network/Fetch error for AccuWeather:', error); return null; }
    }

    function renderCalendarList(events) {
        if (!elements.calendarContainer) return;
        elements.calendarContainer.innerHTML = '';
        if (!events || events.length === 0) { elements.calendarContainer.innerHTML = config.uiMessages.noEvents; return; }
        const eventList = document.createElement('ul'); eventList.className = 'events-list';
        const eventsToDisplay = events.slice(0, config.display.maxEventsToShowInList);
        eventsToDisplay.forEach(event => {
            const listItem = document.createElement('li');
            if (event.calendarSource === config.calendars.holidays) listItem.classList.add('holiday-event');
            const eventTitle = event.summary || 'Untitled Event';
            const eventStartDateString = event.start?.dateTime || event.start?.date;
            const eventDescription = event.description || '';
            const eventLocation = event.location || '';
            const formattedDate = formatEventDateParts(eventStartDateString).full;
            listItem.innerHTML = `<strong class="event-title">${escapeHtml(eventTitle)}</strong> <span class="event-date">${escapeHtml(formattedDate)}</span> ${eventLocation ? `<p class="event-location"><i class="fas fa-map-marker-alt"></i> ${escapeHtml(eventLocation)}</p>` : ''} ${eventDescription ? `<p class="event-description">${escapeHtml(eventDescription).replace(/\n/g, '<br>')}</p>` : ''}`;
    ...
    function updateMarqueeContent() {
        ...
        let marqueeItemStrings = [];
        if (appState.calendarEvents.length > 0) {
            const nextEvent = appState.calendarEvents[0];
            const eventTitle = nextEvent.summary || 'Upcoming Event';
            const eventStartDateString = nextEvent.start?.dateTime || nextEvent.start?.date;
            const dateParts = formatEventDateParts(eventStartDateString);
            const eventLink = nextEvent.htmlLink || '#calendar';
            marqueeItemStrings.push(
                `<a href="${escapeHtml(eventLink)}" target="_blank" rel="noopener noreferrer"><span class="latest-label">EVENT:</span> ${escapeHtml(eventTitle)} (${dateParts.month} ${dateParts.day})</a>`
            );
        }
        if (appState.newsItems.length > 0) {
            const newsItem = appState.newsItems[0];
            const newsLink = newsItem.link || '#news';
             marqueeItemStrings.push(
                `<a href="${escapeHtml(newsLink)}"><span class="latest-label">NEWS:</span> ${escapeHtml(newsItem.headline)}</a>`
             );
        }
        if (appState.weatherData && appState.weatherData.temp !== null && appState.weatherData.temp !== undefined) {
            const temp = Math.round(appState.weatherData.temp); const unit = appState.weatherData.unit;
            const desc = appState.weatherData.description ? `, ${escapeHtml(appState.weatherData.description)}` : '';
            const location = appState.weatherData.location ? ` in ${escapeHtml(appState.weatherData.location)}` : '';
            let feelsLikeString = '';
            if (appState.weatherData.feelsLike !== null && appState.weatherData.feelsLike !== undefined) {
                const feelsLikeTemp = Math.round(appState.weatherData.feelsLike);
                feelsLikeString = `${desc ? ' ' : ', '} (Feels like ${feelsLikeTemp}${unit})`;
            }
            marqueeItemStrings.push(
                `<a href="#weather"><span class="latest-label">WEATHER:</span> ${temp}${unit}${desc}${feelsLikeString}${location}</a>`
            );
        } else {
             marqueeItemStrings.push(
                `<a href="#weather"><span class="latest-label">WEATHER:</span> Unavailable</a>`
             );
        }

        const finalMarqueeHtml = marqueeItemStrings.join('');

        elements.marqueeInnerElements.forEach(element => {
            element.innerHTML = finalMarqueeHtml || 'Loading data...';
        });
    }
    ...
    async function processCalendarData() {
        ...
        try {
            ...
            let combinedEvents = results.filter(r => r.status === 'fulfilled' && Array.isArray(r.value)).flatMap(r => r.value);
            const filteredEvents = combinedEvents.filter(event => {
                if (event.calendarSource === config.calendars.university) return true;
            ...
            filteredEvents.sort((a, b) => { const dateA = getComparableDate(a.start); const dateB = getComparableDate(b.start); if (!dateA && !dateB) return 0; if (!dateA) return 1; if (!dateB) return -1; return dateA - dateB; });
    ...
    function openMenu() { if (elements.mobileMenu) { elements.mobileMenu.classList.remove('hidden'); elements.mobileMenu.classList.add('visible'); elements.menuBtn?.setAttribute('aria-expanded', 'true'); } }
    function closeMenu() { if (elements.mobileMenu) { elements.mobileMenu.classList.remove('visible'); elements.mobileMenu.classList.add('hidden'); elements.menuBtn?.setAttribute('aria-expanded', 'false'); } }
    function setupMenuListeners() {
        ...
        elements.mobileNavLinks.forEach(link => link.addEventListener('click', closeMenu));
        document.addEventListener('click', (event) => { if (elements.mobileMenu?.classList.contains('visible') && !elements.mobileMenu.contains(event.target) && event.target !== elements.menuBtn && !elements.menuBtn?.contains(event.target)) { closeMenu(); } });
    }
